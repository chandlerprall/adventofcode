...
class Bot {
	constructor(id, type) {
		this.id = id;
		this.type = type;
		this.chips = new MinHeap();
		this.targets = [];
	}

	run() {
		if (this.chips.heap.length === 2) {
			const lowValue = this.chips.pop();
			const highValue = this.chips.pop();

			this.targets[0].receive(lowValue);
			this.targets[1].receive(highValue);
		}
	}
...
function getBot(id) {
	if (bots.hasOwnProperty(id)) return bots[id];
	return bots[id] = new Bot(id, 'bot');
}

function getOutput(id) {
	if (outputs.hasOwnProperty(id)) return outputs[id];
	return outputs[id] = new Bot(id, 'output');
}
...
.forEach(line => {
	if (line.indexOf('value') === 0) {
		// bot initial assignment
		let [, value, botId] = line.match(/(\d+).*?(\d+)/);
		...
		// bot instruction
		let [, srcBotId, lowDestType, lowDestId, highDestType, highDestId] = line.match(/bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)/);
		srcBotId = parseInt(srcBotId, 10);
		lowDestId = parseInt(lowDestId, 10);
		highDestId = parseInt(highDestId, 10);

		const srcBot = getBot(srcBotId);
		srcBot.targets.push(lowDestType === 'bot' ? getBot(lowDestId) : getOutput(lowDestId));
		srcBot.targets.push(highDestType === 'bot' ? getBot(highDestId) : getOutput(highDestId));
	}
});

Object.keys(bots).forEach(botId => bots[botId].run());

console.log(getOutput(0).chips.peek() * getOutput(1).chips.peek() * getOutput(2).chips.peek());