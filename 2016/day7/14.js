	...
	.map(line => {
		...
		line.split('').forEach(char => {
			if (char === '[') {
				...
				seq.push(char);
				if (seq.length === 4) seq.shift(1);
				if (seq[0] === seq[2] && seq[0] !== seq[1]) {
					if (inHypernet) {
						hypernetBlocks.push(seq.join(''));
					} else {
						supernetBlocks.push(seq.join(''))
					}
				}
			}
		});

		let isSSL = supernetBlocks.reduce(
			(isSSL, supernetBlock) => {
				if (isSSL) return isSSL;

				return hypernetBlocks.reduce(
					(hasMatchingHypernetBlock, hypernetBlock) => {
						return hasMatchingHypernetBlock || (hypernetBlock[0] === supernetBlock[1] && hypernetBlock[1] === supernetBlock[0])
					},
					false
				);
			},
