...
console.log(
	...
	.filter(x => x)
	.map(line => {
		const [, room, sector, checksum] = line.match(/([a-z\-]+)\-(\d+)\[(.*?)\]/)
		return {room: room.replace(/\-/g, ''), sector: parseInt(sector, 10), checksum};
	})
	.filter(info => {
		let charCounts = {};

		info.room.split('').forEach(char => {
			charCounts[char] = charCounts[char] || 0;
			charCounts[char]++;
		});

		const realChecksum = Object.keys(charCounts).sort((charA, charB) => {
			const charACount = charCounts[charA];
			const charBCount = charCounts[charB];

			if (charACount < charBCount) return 1;
			if (charACount > charBCount) return -1;

			return charA.charCodeAt(0) < charB.charCodeAt(0) ? -1 : 1;
		}).slice(0, 5).join('');

		return realChecksum === info.checksum;
	})
	.reduce((sum, info) => {
		return sum + info.sector;
	}, 0)
);