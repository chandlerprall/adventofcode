...
function shift(text, amount) {
	...
	return text
		...
		.map(char => {
			...
			const charNum = originalCharCode - 97;
			const newChar = (charNum + amount) % 26;
			const newCharCode = newChar + 97;
			return String.fromCharCode(newCharCode);
		})
		.join('');
}

// console.log(shift('qzmt', 343))
// process.exit(0);
...
console.log(JSON.stringify(
	...
	.filter(x => x)
	.map(line => {
		let [, room, sector, checksum] = line.match(/([a-z\-]+)\-(\d+)\[(.*?)\]/)
		sector = parseInt(sector, 10);
		room = room.replace(/\-/g, '');
		const decryptedRoom = shift(room, sector);
		return {room, sector, checksum, decryptedRoom};
	})
	.filter(info => info.decryptedRoom.indexOf('northpoleobjects') !== -1)
